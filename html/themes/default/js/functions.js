    ...
    $.ajax({
        ...
        success: function (data) {
            if (data['status'] == 'success') {
                ...
                deferred.resolve(data['data'], data['icons']);
            } else {
                // Application error
                logger(1, 'DEBUG: application error (' + data['status'] + ') on ' + type + ' ' + url + ' (' + data['message'] + ').');
                deferred.reject(data['message']);
    ...
    $.when(getTemplates(null)).done(function (templates) {
        ...
        $('#form-node-template').change(function (e2) {
            ...
            if (template != '') {
                ...
                var templatePromise = getTemplates(template);
                var nodePromise = (action === 'edit') ? getNodes(id) : $.Deferred().resolve(null).promise();

                $.when(templatePromise, nodePromise).done(function (template_values, node_values) {
...
function printLabTopology() {
    ...
    $.when(
        ...
        var networkImgs = [];
        var nodesImgs = [];
        var textObjectsCount = Object.keys(textObjects).length;
    ...
    $.when(
        ...
        $.when(linksourcedata['interfaces'], linktargetdata['interfaces']).done(function (sourceif, targetif) {
            ...
            html = '<form id="addConn" class="addConn-form">' +
                ...
                '<img src="' + '/images/' + ((linksourcetype == 'net') ? 'net_' : '') + 'icons/' + linksourcedata['icon'] + '" class="' + linksourcestyle + ' img-responsive" style="margin:0 auto;">' +
                ...
                '<img src="/images/' + ((linktargettype == 'net') ? 'net_' : '') + 'icons/' + linktargetdata['icon'] + '" class="' + linktargetstyle + ' img-responsive" style="margin:0 auto;">' +
