...
$(document).on('click', '.action-nodestart, .action-nodesstart, .action-nodestart-group', function (e) {
    ...
    // Если запускаем одну конкретную ноду
    if (node_id != null) {
        $.when(getNodes(node_id)).done(function (node) {
            addMessage('info', 'Start node: ' + node['name']);
    ...
    // Если режим выбора — использовать выбранные ноды
    else if (isFreeSelectMode) {
        var nodeLenght = window.freeSelectedNodes.length;
        addMessage('info', 'Start selected nodes...');
        $.when(recursive_start(window.freeSelectedNodes, nodeLenght)).done(function () {
        }).fail(function (message) {
            addMessage('danger', 'Start selected: Error');
        });
    }

    // Если запускаем все ноды
    else if (startAll) {
        $.when(getNodes(null)).done(function (nodes) {
...
$(document).on('click', '.action-nodestop, .action-nodesstop, .action-nodestop-group', function (e) {
    ...
    // Свободный выбор
        ...
        var nodeLenght = window.freeSelectedNodes.length;
        addMessage('info', 'Stop selected nodes...');
        $.when(recursive_stop(window.freeSelectedNodes, nodeLenght)).done(function () {
            // Опционально: успех можно отдельно обработать
        }).fail(function (message) {
            addMessage('danger', 'Stop selected: Error');
        });
    }
...
$(document).on('click', '.action-nodewipe, .action-nodeswipe, .action-nodewipe-group', function (e) {
    ...
    $('#node_wipe').on('click', function (ev) {
        ...
        if (node_id != null) {
            $.when(getNodes(node_id)).done(function (nodes) {
                addMessage('info', 'Wipe node: ' + nodes['name']);
                $.when(wipe(node_id)).done(function () {
                    addMessage('success', nodes['name'] + ': ' + MESSAGES[78]);
                }).fail(function (message) {
                    addMessage('danger', nodes['name'] + ': ' + message);
                });
            }).fail(function (message) {
                addModalError(message);
            });
            ...
            var nodeLenght = window.freeSelectedNodes.length;
            addMessage('info', 'Wipe selected nodes...');
            $.when(recursive_wipe(window.freeSelectedNodes, nodeLenght)).done(function () {
            }).fail(function (message) {
                addMessage('danger', 'Wipe selected: Error');
            });
        }
            ...
            $.when(getNodes(null)).done(function (nodes) {
                var nodeLenght = Object.keys(nodes).length;
                addMessage('info', 'Wipe all nodes...');
                $.when(recursive_wipe(nodes, nodeLenght)).done(function () {
                }).fail(function (message) {
                    addMessage('danger', 'Wipe all: Error');
                });
            }).fail(function (message) {
                addModalError(message);
            });
