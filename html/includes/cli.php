...
function export($node_id, $n, $lab, $uid)
{
	...
	switch ($n->getNType()) {
		...
		case 'vpcs':
			...
			break;
		case 'iol':
			$nvram = $n->getRunningPath() . '/nvram_' . sprintf('%05d', $node_id);
			if (!is_file($nvram)) {
				// NVRAM file not found
				error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][80066] . 'for iol ID ' . $node_id);
				return 80066;
			}
		...
		case 'qemu':
			if ($n->getStatus() < 2 || !isset($GLOBALS['node_config'][$n->getTemplate()])) {
				...
				$cmd = '/opt/unetlab/config_scripts/' . $GLOBALS['node_config'][$n->getTemplate()] . ' -a get -p ' . $n->getPort() . ' -f ' . $tmp . ' -t ' . $timeout;
				exec($cmd, $o, $rc);
				error_log(date('M d H:i:s ') . 'INFO: exporting ' . $cmd);
				if ($rc != 0) {
					error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][80060]);
...
function prepareNode($n, $id, $t, $nets)
{
	...
	if (!is_file($n->getRunningPath() . '/.prepared') && !is_file($n->getRunningPath() . '/.lock')) {
		...
		if ($n->getConfig() == '1' && $n->getConfigData() != '') {
			...
			if (!dumpConfig($n->getConfigData(), $n->getRunningPath() . '/startup-config')) {
				...
				if (substr($n->getTemplate(), 0, 6) == "timos-" || substr($n->getTemplate(), 0, 9) == "timoscpm-" || substr($n->getTemplate(), 0, 9) == "timosiom-") {
					$tmpl = strtok($n->getTemplate(), "-");
				} else {
					$tmpl = $n->getTemplate();
				}
				...
				if (isset($node_prep[$tmpl])) {
					$cmd = '/opt/unetlab/config_scripts/' . $node_prep[$tmpl] . ' ' . $n->getRunningPath();
					error_log(date('M d H:i:s ') . 'DEBUG: run command ' . $cmd);
					exec($cmd, $o, $rc);
				}
			}
		}
	...
	if (isset($node_init[$n->getTemplate()])) {
		$cmd = '/opt/unetlab/config_scripts/' . $node_init[$n->getTemplate()] . ' ' . $n->getRunningPath();
		error_log(date('M d H:i:s ') . 'DEBUG: run command ' . $cmd);
		exec($cmd, $o, $rc);
	}
...
function start($n, $id, $t, $nets, $scripttimeout)
{
	...
	switch ($n->getNType()) {
		...
		case 'qemu':
			if (!is_dir($n->getRunningPath() . '/jail')) {
				$cmd = 'cp -ar /opt/unetlab/skeleton/* ' . $n->getRunningPath() . '/';
				error_log(date('M d H:i:s ') . 'cmd = ' . $cmd);
				exec($cmd, $o, $rc);
				if ($rc !== 0) {
					// Cannot make prepare jail structure
					error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][80091]);
					error_log(date('M d H:i:s ') . implode("\n", $o));
					return 80091;
				}
			}
			// Mount jail ressouces
			$cmd = "mountpoint -q " . $n->getRunningPath() . "/jail || mount -B -o ro /opt/unetlab/jail " . $n->getRunningPath() . "/jail";
			error_log(date('M d H:i:s ') . 'cmd = ' . $cmd);
			exec($cmd, $o, $rc);
			if ($rc !== 0) {
				// Cannot mount jail ressouces
				error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][80092]);
				error_log(date('M d H:i:s ') . implode("\n", $o));
				return 80092;
			}
			// Creating missing qemu dir if needed
			$cmd = "for i in /opt/qemu-*; do ( ls " . $n->getRunningPath() . "/\$i || mkdir " . $n->getRunningPath() . "/\$i) ; done ";
			error_log(date('M d H:i:s ') . 'cmd = ' . $cmd);
			exec($cmd, $o, $rc);
			if ($rc !== 0) {
				error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][80092]);
			...
			// Mount qemu
			$cmd = "for i in /opt/qemu-*; do ( mountpoint -q " . $n->getRunningPath() . "/\$i || mount -B -o ro \$i " . $n->getRunningPath() . "/\$i ) ; done ";
			error_log(date('M d H:i:s ') . 'cmd = ' . $cmd);
			exec($cmd, $o, $rc);
			if ($rc !== 0) {
				error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][80092]);
			...
			// Mount images
			$cmd = " mountpoint -q " . $n->getRunningPath() . "/opt/unetlab/addons/qemu || mount -B -o ro /opt/unetlab/addons/qemu " . $n->getRunningPath() . "/opt/unetlab/addons/qemu";
			error_log(date('M d H:i:s ') . 'cmd = ' . $cmd);
			exec($cmd, $o, $rc);
			if ($rc !== 0) {
				error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][80092]);
	...
	if (($n->getTemplate() == 'timos' || $n->getTemplate() == 'timoscpm' || strtok($n->getTemplate(), "-") == "timos" || strtok($n->getTemplate(), "-") == "timoscpm")   && is_file($n->getRunningPath() . '/floppy.img') && !is_file($n->getRunningPath() . '/.configured') && $n->getConfig() != 0) {
		$flags = preg_replace('/Timos:/', 'Timos: primary-config=cf1:config.cfg ', $flags);
		$flags .= ' -hdb floppy.img';
	}
	if (isset($node_cstart[$n->getTemplate()]) && $n->getConfig() != 0  && !is_file($n->getRunningPath() . '/.configured')) {
		$flags .= ' ' . $node_cstart[$n->getTemplate()];
	}
	...
	// Clean TCP port
	exec("fuser -k -n tcp " . (32768 + 256 * $t + $id));
	if ($n->getNType() == 'qemu') { // check vm / bare for vmx passthrough
		if (HYPERVISOR == "vm" && $n->getQemu_arch() == 'x86_64') {
	...
	if ($rcp == 0 && $n->getNType() == 'qemu' && is_file($n->getRunningPath() . '/startup-config') && !is_file($n->getRunningPath() . '/.configured') && $n->getConfig() != 0) {
		...
		$cmd = 'nohup /opt/unetlab/config_scripts/' . $GLOBALS['node_config'][$n->getTemplate()] . ' -a put -p ' . $n->getPort() . ' -f ' . $n->getRunningPath() . '/startup-config -t ' . ($n->getDelay() + $scripttimeout) . ' > /dev/null 2>&1 &';
		exec($cmd, $o, $rc);
	...
	if ($rcp == 0 && $n->getNType() == 'docker') {
		...
		$cmd = 'nohup /opt/unetlab/config_scripts/' . $GLOBALS['node_config'][$n->getTemplate()] . ' -a put -i ' . $n->getUuid() . ' -f ' . $n->getRunningPath() . '/startup-config -t ' . ($n->getDelay() + 300) . ' > /dev/null 2>&1 &';
		exec($cmd, $o, $rc);
		error_log(date('M d H:i:s ') . 'INFO: importing ' . $cmd);
	}
...
/**
 * Function to umount jail for qemu  a node.
 *
 * @param   Node    $n                  Node
 * @return  int                         0 means ok
 */
...
{
	if ($n->getNType() == 'qemu') {
		exec("umount -l -t bind " . $n->getRunningPath() . "/jail  || true ", $o, $rc);
		exec("umount -l -t bind " . $n->getRunningPath() . "/opt/qemu-*  || true ", $o, $rc);
		exec("umount -l -t bind " . $n->getRunningPath() . "/opt/unetlab/addons/qemu  || true ", $o, $rc);
		return 0;
	} else {
		return 0;
	}
