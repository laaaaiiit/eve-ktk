...
$cgroup_version = file_exists('/sys/fs/cgroup/cgroup.controllers') ? 2 : 1;

// Set paths based on cgroup version
if ($cgroup_version === 2) {
    $CGROUP_TASKS_PATH = '/sys/fs/cgroup/cpulimit/cgroup.procs';
} else {
    $CGROUP_TASKS_PATH = '/sys/fs/cgroup/pids/cpulimit/tasks';
}
...
if ($cgroup_version === 2 && !file_exists('/sys/fs/cgroup/cpulimit')) {
    system('mkdir -p /sys/fs/cgroup/cpulimit');
    system('chown www-data:www-data /sys/fs/cgroup/cpulimit');
    system('echo "+pids +cpu" > /sys/fs/cgroup/cgroup.subtree_control');
}
...
while ( true ) {
        ...
        foreach ( $o as $pidsmp ) {
                ...
                // Check if process is in cgroup (compatible with both v1 and v2)
                exec("grep ".$pid." ".$CGROUP_TASKS_PATH." 2>/dev/null",$ou,$rc);
                if ( $rc != 0 ) {
                        continue;
                }
                if ( $cpu > $max ) {
                         if ( isset($status[$pid]) && $status[$pid] == 0 && $cpu > $UNLIMIT*$smp ) {
                                ...
                                eve_log ("limit already active but cpu high pid: ".$pid." smp:".$smp." max: ".$max." usage: ".$cpu."\n" );
                                ...
                                // twice over limit ... need to limit now !!!
