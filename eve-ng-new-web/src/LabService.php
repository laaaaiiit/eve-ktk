...
function listViewerCloudProfiles(PDO $db, string $viewerUserId): array
{
    if ($viewerUserId === '') {
        return [];
    }
...
function formatLabPortDisplayName(string $nodeType, string $portName): string
{
    ...
    if ($nodeType === 'iol' && preg_match('/^eth([0-9]+)$/i', $portName, $m) === 1) {
        $idx = (int) $m[1];
        $slot = (int) floor($idx / 4);
        $sub = $idx % 4;
        return 'eth' . $slot . '/' . $sub;
    }
    return $portName;
...
function v2TemplateDir(): string
{
    ...
    return '/opt/unetlab/html/templates/' . $platform;
}

function listNodeIconsV2(): array
{
    $iconsDir = '/opt/unetlab/html/images/icons';
    $result = [];
    if (!is_dir($iconsDir)) {
        return $result;
    }
...
{
    $configsDir = '/opt/unetlab/html/configs';
    $result = [];
...
function listNodeImagesV2(string $type, string $template, bool $allowFallback = true): array
{
    ...
    if ($type === 'iol') {
        $dir = '/opt/unetlab/addons/iol/bin';
        if (is_dir($dir)) {
            foreach (scandir($dir) ?: [] as $filename) {
                if (preg_match('/\.bin$/i', $filename) && is_file($dir . '/' . $filename)) {
        ...
        $dir = '/opt/unetlab/addons/dynamips';
        if (is_dir($dir)) {
            $allImages = [];
            foreach (scandir($dir) ?: [] as $filename) {
                ...
                if (preg_match('/\.image$/i', $filename)) {
                    $allImages[] = $filename;
                }
                if (preg_match('/^' . preg_quote($template, '/') . '-.+\.image$/i', $filename)) {
                    $result[$filename] = $filename;
                }
            }
            if ($allowFallback && empty($result) && !empty($allImages)) {
                $token = preg_quote($template, '/');
                foreach ($allImages as $filename) {
                    if (preg_match('/(^|[-_.])' . $token . '([-.]|$)/i', $filename)) {
                        $result[$filename] = $filename;
                    }
                }
            }
...
function getNodeTemplateOptionsV2(string $template): array
{
    ...
    if ($type === 'dynamips') {
        $options['idlepc'] = ['type' => 'input', 'value' => (string) ($tpl['idlepc'] ?? '')];
        $options['nvram'] = ['type' => 'input', 'value' => (int) ($tpl['nvram'] ?? 128)];
    }
    if ($type === 'iol') {
        $options['nvram'] = ['type' => 'input', 'value' => (int) ($tpl['nvram'] ?? 256)];
        $options['ethernet'] = ['type' => 'input', 'value' => (int) ($tpl['ethernet'] ?? 1)];
        $options['serial'] = ['type' => 'input', 'value' => (int) ($tpl['serial'] ?? 0)];
    }
    if ($type === 'docker') {
        $options['ethernet'] = ['type' => 'input', 'value' => (int) ($tpl['ethernet'] ?? 1)];
    }
    ...
    return [
        ...
        'attachments' => array_map(static function ($row) {
            ...
            return [
                ...
                'port_name' => $portName,
                'port_label' => formatLabPortDisplayName($nodeType, $portName),
...
function createLabNode(PDO $db, array $viewer, string $labId, array $payload): array
{
    ...
    if (!in_array($nodeType, ['qemu', 'iol', 'dynamips', 'docker', 'vpcs'], true)) {
    ...
    if ($isIolNode) {
        ...
        if ($serialCount < 0 || $serialCount > 16) {
            throw new InvalidArgumentException('serial_count_invalid');
        }
        if (($ethernetCount + $serialCount) > 16) {
            throw new InvalidArgumentException('iol_slot_limit_exceeded');
        }
    ...
    $ethernetPortCount = $isIolNode ? ($ethernetCount * 4) : $ethernetCount;
    ...
    try {
        ...
        for ($idx = 0; $idx < $count; $idx++) {
            $nodeLeft = $left + (($idx % 5) * 60);
            $nodeTop = $top + ((int) floor($idx / 5) * 80);
...
function updateLabNode(PDO $db, array $viewer, string $labId, string $nodeId, array $payload): array
{
    ...
    $ethernetPortCount = ($nodeType === 'iol') ? ($ethernetCount * 4) : $ethernetCount;
...
function listLabPortsForViewer(PDO $db, array $viewer, string $labId): array
{
    ...
    return array_map(static function (array $row): array {
        ...
        return [
            ...
            'name' => $name,
            'display_name' => formatLabPortDisplayName($nodeType, $name),
...
{
    // Link changes must not force-restart running nodes.
    // Runtime relink is handled in-process without stop/start tasks.
    $unique = [];
...
function resolveLabCloudInput(PDO $db, string $viewerUserId, array $payload): array
{
    if ($viewerUserId === '') {
        throw new RuntimeException('Forbidden');
    }
...
function listLabCloudOptions(PDO $db, array $viewer, string $labId): array
{
    ...
    return listViewerCloudProfiles($db, $viewerUserId);
}
...
function createLabCloudNetwork(PDO $db, array $viewer, string $labId, array $payload): array
{
    ...
    $resolved = resolveLabCloudInput($db, (string) ($viewer['id'] ?? ''), $payload);
    $left = max(0, (int) ($payload['left'] ?? 0));
    $top = max(0, (int) ($payload['top'] ?? 0));
...
function normalizeLabObjectData(string $objectType, array $currentData, array $payload): array
{
    ...
    if ($objectType === 'text') {
        ...
        return [
            ...
            'top' => $top,
            'text' => $text,
            'width' => labObjectClampInt($payload['width'] ?? ($currentData['width'] ?? 320), 120, 1600, 320),
            'height' => labObjectClampInt($payload['height'] ?? ($currentData['height'] ?? 88), 40, 1000, 88),
            ...
            'z_index' => labObjectClampInt($payload['z_index'] ?? ($currentData['z_index'] ?? 30), 1, 10000, 30),
        ];
    }
    ...
    return [
        ...
        'top' => $top,
        'shape' => $shape,
        'width' => labObjectClampInt($payload['width'] ?? ($currentData['width'] ?? 220), 30, 2000, 220),
        'height' => labObjectClampInt($payload['height'] ?? ($currentData['height'] ?? 140), 30, 2000, 140),
        ...
        'z_index' => labObjectClampInt($payload['z_index'] ?? ($currentData['z_index'] ?? 20), 1, 10000, 20),
    ];
}
...
function createLabNodeNetworkLink(PDO $db, array $viewer, string $labId, array $payload): array
{
    ...
    try {
        ...
        if (!empty($sourcePort['network_id'])) {
            throw new RuntimeException('port_in_use');
        }
