...
$(document).on('change', '.change_config_status', function () {
    ...
    var align_flash = $(this).closest('li').find('.absolute-right').find('span.align-flash');
    if(!$(this).prop('disabled')) {
        if (!$(this).prop('checked')) {
            setStartupData(id, true, 0, name);
            align_flash.html('&nbsp;&nbsp;&nbsp;');
        } else {
            setStartupData(id, true, 1, name);
            align_flash.html('<i class="glyphicon glyphicon-flash tg-flash"></i>');
        }
    }
});
...
$(window).resize(function () {
    ...
    viewport.width(($(window).width()-40) * (100/$('#zoomslide').slider("value")));
    viewport.height($(window).height() * (100/$('#zoomslide').slider("value")));
