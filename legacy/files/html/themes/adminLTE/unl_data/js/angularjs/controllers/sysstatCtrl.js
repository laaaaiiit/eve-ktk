angular.module("unlMainApp").controller('sysstatController',function sysstatController($scope, $http, $rootScope, $interval, $location, $cookies, $timeout, themeService) {
        var translations = {
                en: {
                        ...
                        gaugeCpuMetaCores: 'Cores',
                        gaugeCpuMetaFreq: 'Frequency',
                        gaugeMemLabel: 'Memory',
                        gaugeMemSub: 'Allocated RAM',
                        gaugeMemMetaTotal: 'Allocated',
                        gaugeMemMetaUsed: 'Used',
                        gaugeSwapLabel: 'Swap',
                ...
                ru: {
                        ...
                        gaugeCpuMetaCores: 'Ядер',
                        gaugeCpuMetaFreq: 'Частота',
                        gaugeMemLabel: 'Память',
                        gaugeMemSub: 'Выделенная RAM',
                        gaugeMemMetaTotal: 'Выделено',
                        gaugeMemMetaUsed: 'Используется',
                        gaugeSwapLabel: 'Swap',
	...
	$scope.systemstat = function(){
		$http.get('/api/status').then(
				function successCallback(response) {
					...
					$scope.cpuMeta.count = (typeof $scope.serverstatus.cpu_count !== 'undefined' && $scope.serverstatus.cpu_count !== null)
						? String($scope.serverstatus.cpu_count)
						: '--';
					$scope.cpuMeta.ghz = (typeof $scope.serverstatus.cpu_ghz !== 'undefined' && $scope.serverstatus.cpu_ghz !== null && Number($scope.serverstatus.cpu_ghz) > 0)
						? (Number($scope.serverstatus.cpu_ghz).toFixed(2) + ' GHz')
						: '--';
					$scope.memMeta.total = formatGiBFromKb($scope.serverstatus.mem_total_kb);
					$scope.memMeta.used = formatGiBFromKb($scope.serverstatus.mem_used_kb);
