...
/**
 * Return wrapper path for new runtime only.
 *
 * @return string
 */
function unlWrapperPath()
{
	$path = '/opt/unetlab/wrappers-v2/unl_wrapper';
	if (!is_file($path)) {
		error_log(date('M d H:i:s ') . 'ERROR: new runtime wrapper not found at ' . $path);
	}
	return $path;
}
...
function isVirtual()
{
	switch (FORCE_VM) {
		default:
			...
			$cmd = 'sudo ' . escapeshellarg(unlWrapperPath()) . ' -a platform';
			exec($cmd, $o, $rc);
			$o = implode('', $o);
			switch ($o) {
				default:
