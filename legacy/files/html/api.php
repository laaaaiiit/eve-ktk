...
$app->get('/api/status', function () use ($app, $db) {
	...
	$cpuInfo = apiGetCpuHardwareInfo();
	$output['data']['cpu_count'] = isset($cpuInfo['count']) ? (int) $cpuInfo['count'] : 0;
	$output['data']['cpu_ghz'] = isset($cpuInfo['ghz']) ? (float) $cpuInfo['ghz'] : 0.0;
	$output['data']['disk'] = apiGetDiskUsage();
	list($output['data']['cached'], $output['data']['mem']) = apiGetMemUsage();
	$memDetails = apiGetMemDetails();
	$output['data']['mem_total_kb'] = isset($memDetails['total_kb']) ? (int) $memDetails['total_kb'] : 0;
	$output['data']['mem_used_kb'] = isset($memDetails['used_kb']) ? (int) $memDetails['used_kb'] : 0;
	$output['data']['mem_available_kb'] = isset($memDetails['available_kb']) ? (int) $memDetails['available_kb'] : 0;
...
$app->delete('/api/status', function () use ($app, $db) {
	...
	$cmd = 'sudo ' . escapeshellarg(unlWrapperPath()) . ' -a stopall';
	exec($cmd, $o, $rc);
	if ($rc != 0) {
		error_log(date('M d H:i:s ') . 'ERROR: ' . $GLOBALS['messages'][60044]);
		$output['code'] = 400;
...
$app->get('/api/update', function () use ($app, $db) {
	...
	$cmd = 'sudo ' . escapeshellarg(unlWrapperPath()) . ' -a update';
	exec($cmd, $o, $rc);
	if ($rc != 0) {
