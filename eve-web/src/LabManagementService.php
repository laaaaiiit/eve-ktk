...
function listLabManagementOverview(PDO $db): array
{
    ...
    $nodeTypeStmt = $db->query(
        "SELECT
            LOWER(COALESCE(node_type, '')) AS node_type,
            LOWER(COALESCE(template, '')) AS template,
            LOWER(COALESCE(image, '')) AS image,
            COUNT(*) AS cnt,
            COUNT(*) FILTER (WHERE LOWER(COALESCE(power_state, '')) = 'running') AS running_cnt
         FROM lab_nodes
         GROUP BY
            LOWER(COALESCE(node_type, '')),
            LOWER(COALESCE(template, '')),
            LOWER(COALESCE(image, ''))"
    );
