...
function userPasswordMinLength(): int
{
    $value = (int) getenv('EVE_PASSWORD_MIN_LENGTH');
    if ($value < 8) {
        $value = 8;
    }
    return $value;
...
function userHashPassword(string $password): string
{
    if (defined('PASSWORD_ARGON2ID')) {
        $memoryCost = (int) getenv('EVE_ARGON2_MEMORY_COST');
        $timeCost = (int) getenv('EVE_ARGON2_TIME_COST');
        $threads = (int) getenv('EVE_ARGON2_THREADS');
        if ($memoryCost < 65536) {
            $memoryCost = 65536;
        }
        if ($timeCost < 2) {
...
function userPasswordNeedsRehash(string $hash): bool
{
    if (defined('PASSWORD_ARGON2ID')) {
        return password_needs_rehash($hash, PASSWORD_ARGON2ID, [
            'memory_cost' => max(65536, (int) getenv('EVE_ARGON2_MEMORY_COST')),
            'time_cost' => max(2, (int) getenv('EVE_ARGON2_TIME_COST')),
            'threads' => max(1, (int) getenv('EVE_ARGON2_THREADS')),
        ]);
    }
    return password_needs_rehash($hash, PASSWORD_BCRYPT);
}

function maybeUpgradeUserPasswordHash(PDO $db, string $userId, string $password, string $currentHash): bool
{
    if (trim($userId) === '' || trim($currentHash) === '') {
        return false;
    }
    if (!userPasswordNeedsRehash($currentHash)) {
        return false;
    }
    $newHash = userHashPassword($password);
    $stmt = $db->prepare('UPDATE users SET password_hash = :password_hash, updated_at = NOW() WHERE id = :id');
    $stmt->bindValue(':id', $userId, PDO::PARAM_STR);
    $stmt->bindValue(':password_hash', $newHash, PDO::PARAM_STR);
    $stmt->execute();
    return true;
}
...
function createUser(PDO $db, string $username, string $password, string $roleId, bool $isBlocked, string $lang, string $theme): array
{
    ...
    $hash = userHashPassword($password);
...
function updateUser(PDO $db, string $userId, string $roleId, bool $isBlocked, ?string $newPassword, string $lang, string $theme): bool
{
    ...
    try {
        if ($newPassword !== null && $newPassword !== '') {
            if (strlen($newPassword) < userPasswordMinLength()) {
                throw new InvalidArgumentException('password_too_short');
            }
            $hash = userHashPassword($newPassword);
...
function getUserPasswordHash(PDO $db, string $userId): ?string
{
...
function updateOwnPassword(PDO $db, string $userId, string $newPassword): bool
{
    if (strlen($newPassword) < userPasswordMinLength()) {
        throw new InvalidArgumentException('password_too_short');
    }
