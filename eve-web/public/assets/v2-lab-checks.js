(function () {
  ...
  const state = {
    ...
    activeRun: null,
    regexHelperRules: [],
    regexHelperStatus: { kind: 'idle', text: '' }
  };
  ...
  function apiLabTasks() {
    const labId = String((ctx.getLabId && ctx.getLabId()) || '').trim();
    return `/api/labs/${encodeURIComponent(labId)}/tasks?limit=200`;
  }
  ...
  function currentUserId() {
    ...
    return String(me && me.id ? me.id : '').trim();
  }
  ...
  function errorLabel(errorText) {
    ...
    const map = {
      ...
      windows_agent_unavailable: 'labChecksErrWindowsAgentUnavailable',
      windows_agent_exec_failed: 'labChecksErrWindowsAgentExecFailed',
      windows_agent_timeout: 'labChecksErrWindowsAgentTimeout',
  ...
  function normalizeConfigPayload(payload) {
    ...
    return {
      ...
      items: items.map((item) => ({
        ...
        is_required: boolOf(item && item.is_required),
  ...
  function setRunBusyUi(isBusy) {
    const busy = !!isBusy;
    ['labChecksRunBtn', 'labChecksSaveBtn', 'labChecksOpenGradesModalBtn', 'labChecksRefreshRunsBtn', 'labChecksExportBtn', 'labChecksConfigImportBtn', 'labChecksConfigExportBtn', 'labChecksSyncCopiesBtn', 'labChecksRegexHelperBtn'].forEach((id) => {
      const el = byId(id);
      if (!el) return;
      el.disabled = busy;
  ...
  async function pollRunProgressTick() {
    ...
    try {
      ...
      try {
        const taskRes = await ctx.jget(apiLabTasks());
        const taskRows = Array.isArray(taskRes && taskRes.data) ? taskRes.data : [];
        trackedTask = selectTrackedLabCheckTask(taskRows);
      } catch (_) {
        trackedTask = null;
      }

      if (trackedTask) {
        state.runTaskId = String(trackedTask && trackedTask.id ? trackedTask.id : '').trim();
        state.runTaskStatus = String(trackedTask && trackedTask.status ? trackedTask.status : '').toLowerCase();
        const taskResult = (trackedTask && trackedTask.result_data && typeof trackedTask.result_data === 'object')
          ? trackedTask.result_data
          : {};
        const runIdFromResult = String(taskResult && taskResult.run && taskResult.run.id ? taskResult.run.id : '').trim();
        const runIdFromProgress = String(trackedTask && trackedTask.run_id ? trackedTask.run_id : '').trim();
        const runIdFromTask = runIdFromResult || runIdFromProgress;
        if (!state.runObservedId && runIdFromTask) {
          state.runObservedId = runIdFromTask;
        }
      } else {
        // If a specific task is already tracked, keep waiting instead of falling back to stale completed entries.
        state.runTaskStatus = state.runTaskId ? 'pending' : '';
      }
      ...
      if (candidateRunId) {
        ...
        const configuredEnabled = Array.isArray(state.config && state.config.items)
          ? state.config.items.filter((item) => !!(item && item.is_enabled)).length
          : 0;
        ...
        const doneFromRunCounters = Number(run.passed_items || 0) + Number(run.failed_items || 0) + Number(run.error_items || 0);
        const doneFromRows = rows.reduce((acc, row) => {
          const status = String((row && row.status) || '').toLowerCase();
          return acc + ((status === 'passed' || status === 'failed' || status === 'error') ? 1 : 0);
        }, 0);
        const done = Math.min(total, Math.max(0, doneFromRunCounters, doneFromRows));
        const nextProgress = Math.min(100, (done / total) * 100);
        setRunProgress(true, nextProgress, `${tr('labChecksRunInProgress')} ${done}/${total}`);
        syncLiveFeedFromRows(rows);
        renderRunSelect();
        renderRunResult();

        const runStatus = String(run && run.status ? run.status : '').toLowerCase();
        const runFinished = runStatus === 'completed' || runStatus === 'failed';
        const taskStatus = String(state.runTaskStatus || '').toLowerCase();
        const taskFinished = taskStatus === 'done' || taskStatus === 'failed' || taskStatus === '';
        if (runFinished && taskFinished) {
          const ok = runStatus === 'completed';
          const msg = ok ? tr('labChecksRunDone') : tr('labChecksRunProgressFailed');
          await finishRunBusyState(ok, msg);
          return;
        }
      } else {
        const taskStatus = String(state.runTaskStatus || '').toLowerCase();
        if (taskStatus === 'running') {
          setRunProgress(true, Math.max(10, Number(state.runProgressValue || 0)), tr('labChecksRunInProgress'));
        } else if (taskStatus === 'pending') {
          setRunProgress(true, Math.max(4, Number(state.runProgressValue || 0)), tr('labChecksRunInProgress'));
        } else if (taskStatus === 'done') {
          await loadRuns(state.activeRunId || '');
          const latest = Array.isArray(state.runs) && state.runs.length ? state.runs[0] : null;
          if (latest && String(latest.id || '').trim()) {
            state.runObservedId = String(latest.id || '').trim();
            await loadRunDetails(state.runObservedId);
            const run = state.activeRun && state.activeRun.run ? state.activeRun.run : {};
            const ok = String(run.status || '').toLowerCase() === 'completed';
            await finishRunBusyState(ok, ok ? tr('labChecksRunDone') : tr('labChecksRunProgressFailed'));
            return;
          }
          await finishRunBusyState(false, tr('labChecksRunProgressFailed'));
          return;
        } else if (taskStatus === 'failed') {
          const taskError = trackedTask && trackedTask.error_text ? String(trackedTask.error_text) : tr('labChecksRunProgressFailed');
          await finishRunBusyState(false, taskError);
          return;
        }
        renderRunSelect();
      }
  ...
  function isLabCheckTaskForCurrentViewer(task) {
    const action = String(task && task.action ? task.action : '').toLowerCase();
    if (action !== 'lab_check') return false;
    const taskUserId = String(task && task.requested_by_user_id ? task.requested_by_user_id : '').trim();
    const meId = currentUserId();
    if (taskUserId && meId && taskUserId !== meId) return false;
    return true;
  }
  ...
  function selectTrackedLabCheckTask(rows) {
    const list = (Array.isArray(rows) ? rows : [])
      .filter(isLabCheckTaskForCurrentViewer)
      .sort((a, b) => parseDateMs(b && b.created_at) - parseDateMs(a && a.created_at));
    if (!list.length) return null;

    const desiredId = String(state.runTaskId || '').trim();
    if (desiredId) {
      const foundById = list.find((row) => String(row && row.id ? row.id : '').trim() === desiredId);
      if (foundById) return foundById;
      const activeForViewer = list.find((row) => {
        const status = String(row && row.status ? row.status : '').toLowerCase();
        return status === 'pending' || status === 'running';
      });
      if (activeForViewer) return activeForViewer;
      // Do not fallback to arbitrary finished task when an explicit task id is tracked.
      return null;
    }
    ...
    // While actively tracking a run, do not attach to an arbitrary finished
    // task from history (it can prematurely complete current progress UI).
    if (state.runBusy) return null;
    return list[0];
  }

  async function finishRunBusyState(ok, toastMessage) {
    stopRunPollTimer();
    state.runBusy = false;
    setRunBusyUi(false);
    ...
    if (toastMessage) {
      toast(String(toastMessage), ok ? 'success' : 'error');
    }
    try {
      await loadRuns(state.activeRunId || '');
    } catch (_) {}
  ...
  function applyModalTheme() {
    ...
    const gradesModalCard = byId('labChecksGradesModalCard');
    const gradesModalBackdrop = byId('labChecksGradesModalBackdrop');
    const noManage = byId('labChecksNoManage');
    const syncNotice = byId('labChecksSyncNotice');
    ...
    const runsBox = byId('labChecksHistoryBox');
    const resultsBox = byId('labChecksResultsBox');
    const regexModalCard = byId('labChecksRegexModalCard');
    const regexModalBackdrop = byId('labChecksRegexModalBackdrop');
    const regexSamplePanel = byId('labChecksRegexSamplePanel');
    const regexBuildPanel = byId('labChecksRegexBuildPanel');
    const regexGuide = byId('labChecksRegexGuide');
    const regexPresetSelect = byId('labChecksRegexRuleTypePreset');
    const regexSampleInput = byId('labChecksRegexSampleInput');
    const regexPatternOutput = byId('labChecksRegexPatternOutput');
    ...
    const syncNoticeHidden = !!(syncNotice && syncNotice.classList.contains('hidden'));
    ...
    if (theme() === 'light') {
      ...
      if (syncNotice) syncNotice.className = 'mb-3 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800';
      ...
      if (runsBox) runsBox.className = 'mb-3';
      if (resultsBox) resultsBox.className = 'min-h-[240px] max-h-[48vh] overflow-auto xl:min-h-0 xl:max-h-none xl:flex-1';
      if (head) head.className = 'sticky top-0 z-10 bg-slate-100 text-slate-700';
      if (gradesModalCard) gradesModalCard.className = 'relative w-full max-w-[520px] rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 shadow-2xl';
      if (gradesModalBackdrop) gradesModalBackdrop.className = 'absolute inset-0 bg-slate-950/65';
      if (regexModalCard) regexModalCard.className = 'relative flex w-full max-w-[980px] max-h-[88vh] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 shadow-2xl';
      if (regexModalBackdrop) regexModalBackdrop.className = 'absolute inset-0 bg-slate-950/65';
      if (regexGuide) regexGuide.className = 'mb-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700';
      if (regexSamplePanel) regexSamplePanel.className = 'rounded-xl border border-slate-200 bg-slate-50 p-3';
      if (regexBuildPanel) regexBuildPanel.className = 'rounded-xl border border-slate-200 bg-slate-50 p-3';
    } else {
      ...
      if (syncNotice) syncNotice.className = 'mb-3 rounded-xl border border-amber-500/45 bg-amber-500/10 px-3 py-2 text-xs text-amber-200';
      ...
      if (gradesModalCard) gradesModalCard.className = 'relative w-full max-w-[520px] rounded-2xl border border-white/20 bg-slate-900 p-4 text-slate-100 shadow-2xl';
      if (gradesModalBackdrop) gradesModalBackdrop.className = 'absolute inset-0 bg-slate-950/65';
      if (regexModalCard) regexModalCard.className = 'relative flex w-full max-w-[980px] max-h-[88vh] flex-col overflow-hidden rounded-2xl border border-white/20 bg-slate-900 p-4 text-slate-100 shadow-2xl';
      if (regexModalBackdrop) regexModalBackdrop.className = 'absolute inset-0 bg-slate-950/65';
      if (regexGuide) regexGuide.className = 'mb-3 rounded-xl border border-slate-500/35 bg-slate-950/45 px-3 py-2 text-xs text-slate-200';
      if (regexSamplePanel) regexSamplePanel.className = 'rounded-xl border border-slate-500/35 bg-slate-950/45 p-3';
      if (regexBuildPanel) regexBuildPanel.className = 'rounded-xl border border-slate-500/35 bg-slate-950/45 p-3';
    }
    if (noManage) {
      noManage.classList.toggle('hidden', noManageHidden);
    }
    if (syncNotice) {
      syncNotice.classList.toggle('hidden', syncNoticeHidden);
    }
    if (runProgressWrap) {
    ...
    if (itemsPanel) {
      itemsPanel.className = 'flex min-h-0 min-w-0 flex-col overflow-hidden';
    }
    ...
    [regexSampleInput, regexPatternOutput].forEach((input) => {
      if (!input) return;
      input.className = `${inputClass({ compact: false, rounded: 'rounded-xl' })} text-sm`;
    });
    if (regexPresetSelect) {
      regexPresetSelect.className = `${inputClass({ compact: true, rounded: 'rounded-xl' })} w-auto min-w-[176px] text-xs`;
    }
    ...
    repaintAllDropdowns();
    renderRegexTestResult();
  }

  function syncNoticeText(notice) {
    const src = (notice && typeof notice === 'object') ? notice : {};
    const code = String(src.code || '').trim().toLowerCase();
    if (!code) return '';
    let base = code === 'node_mapping_mismatch'
      ? tr('labChecksSyncNoticeNodeMappingMismatch')
      : tr('labChecksSyncNoticeTopologyMismatch');

    const missing = Array.isArray(src.missing_nodes) ? src.missing_nodes.map((v) => String(v || '').trim()).filter(Boolean) : [];
    const ambiguous = Array.isArray(src.ambiguous_nodes) ? src.ambiguous_nodes.map((v) => String(v || '').trim()).filter(Boolean) : [];
    if (missing.length) {
      base += ` ${fillTokens(tr('labChecksSyncNoticeMissingNodes'), { nodes: missing.join(', ') })}`;
    }
    if (ambiguous.length) {
      base += ` ${fillTokens(tr('labChecksSyncNoticeAmbiguousNodes'), { nodes: ambiguous.join(', ') })}`;
    }
    return base.trim();
  }

  function renderSyncNotice() {
    const root = byId('labChecksSyncNotice');
    if (!root) return;
    const text = syncNoticeText(state.syncNotice);
    if (!text) {
      root.textContent = '';
  ...
  function renderGradeRows() {
    ...
    root.innerHTML = grades.map((g, idx) => {
      ...
      return `<div class="js-lab-grade-row grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_34px] items-center gap-1.5 min-w-0 w-full" data-grade-idx="${idx}">
        ...
        <button type="button" class="js-lab-grade-del-btn lab-btn inline-flex h-8 w-8 items-center justify-center rounded-xl border border-rose-300 bg-gradient-to-r from-rose-500 to-red-500 px-0 py-0 text-white hover:from-rose-600 hover:to-red-600" data-grade-del="${idx}">
  ...
  function renderItems() {
    ...
    const nodes = Array.isArray(labData() && labData().nodes) ? labData().nodes : [];
    const nodeOptions = [{ value: '', label: tr('labChecksItemNodeUnset') }].concat(nodes.map((node) => ({
    ...
    root.innerHTML = items.map((item, idx) => {
      ...
      const cardHeaderSpacingClass = collapsed ? 'mb-0' : 'mb-2';
      ...
      return `<article class="js-lab-check-item rounded-xl border p-3" data-check-idx="${idx}" data-check-id="${esc(itemId)}" data-check-collapsed="${collapsed ? '1' : '0'}">
        <div class="${cardHeaderSpacingClass} flex items-center justify-between gap-2">
          <div class="min-w-0 truncate text-xs font-semibold ${previewTone}" title="${esc(titlePreview)}">${esc(titlePreview)}</div>
          <div class="flex items-center gap-2">
            <button type="button" class="js-lab-check-btn-collapse lab-btn lab-btn-compact inline-flex items-center justify-center gap-1.5 rounded-xl border border-blue-300 bg-gradient-to-r from-blue-500 to-sky-500 px-2.5 py-1.5 text-xs font-semibold text-white hover:from-blue-600 hover:to-sky-600 whitespace-nowrap" data-check-toggle="${idx}" aria-expanded="${collapsed ? 'false' : 'true'}" title="${esc(collapseLabel)}">
              <i class="fa ${collapseIcon}"></i>
              <span>${esc(collapseLabel)}</span>
            </button>
            <button type="button" class="js-lab-check-btn-delete lab-btn lab-btn-compact inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-300 bg-gradient-to-r from-rose-500 to-red-500 px-2.5 py-1.5 text-xs font-semibold text-white hover:from-rose-600 hover:to-red-600 whitespace-nowrap" data-check-del="${idx}">
              <i class="fa fa-trash"></i>
        ...
        <div class="js-lab-check-body ${collapsed ? 'hidden' : ''}">
          <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
            <div class="md:col-span-2 grid grid-cols-1 gap-2 md:grid-cols-4">
              <label class="text-xs">
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex flex-wrap items-center gap-3">
              ...
              <label class="inline-flex items-center gap-2">
                <input type="checkbox" class="js-lab-check-cb-input sr-only" data-check-field="is_required" ${item.is_required ? 'checked' : ''}>
                <span class="js-lab-check-cb-host inline-flex"></span>
                <span>${esc(tr('labChecksItemRequired'))}</span>
              </label>
            </div>
            <div class="flex flex-wrap items-center gap-3">
              <label class="inline-flex items-center gap-2">
  ...
  function renderManageSection() {
    ...
    const openGradesBtn = byId('labChecksOpenGradesModalBtn');
    const configImportBtn = byId('labChecksConfigImportBtn');
    const configExportBtn = byId('labChecksConfigExportBtn');
    const syncCopiesBtn = byId('labChecksSyncCopiesBtn');
    const regexHelperBtn = byId('labChecksRegexHelperBtn');
    const setInlineBtnVisible = (el, visible) => {
      if (!el) return;
    ...
    setInlineBtnVisible(openGradesBtn, !!state.canManage);
    setInlineBtnVisible(configImportBtn, !!state.canManage);
    setInlineBtnVisible(configExportBtn, !!state.canManage);
    setInlineBtnVisible(syncCopiesBtn, !!state.canManage);
    setInlineBtnVisible(regexHelperBtn, !!state.canManage);
    renderSyncNotice();
  ...
  function renderRunResult() {
    ...
    const requiredTotal = rows.reduce((acc, item) => acc + (((item && item.is_required) ? 1 : 0)), 0);
    const requiredPassed = rows.reduce((acc, item) => acc + (((item && item.is_required && item.is_passed) ? 1 : 0)), 0);
    const requiredFailed = Math.max(0, requiredTotal - requiredPassed);
    const gradeLabel = String(run && run.grade_label ? run.grade_label : '').trim();
    const summaryParts = [
      `${tr('labChecksScoreSummary')}: ${Number(run.earned_points || 0)}/${Number(run.total_points || 0)} (${Number(run.score_percent || 0).toFixed(2)}%)`,
      `${tr('labChecksRunsCount')}: ${Number(run.total_items || 0)}`
    ];
    if (gradeLabel) {
      summaryParts.splice(1, 0, `${tr('labChecksGradeLabel')}: ${gradeLabel}`);
    }
    if (requiredTotal > 0) {
      summaryParts.push(`${tr('labChecksRequiredProgress')}: ${requiredPassed}/${requiredTotal}`);
    }
    const gradeBlockedByRequired = !gradeLabel && requiredFailed > 0;
    const summaryText = summaryParts.join(' | ');
    if (gradeBlockedByRequired) {
      const warnClass = theme() === 'light'
        ? 'ml-2 text-[11px] font-semibold text-rose-700'
        : 'ml-2 text-[11px] font-semibold text-rose-300';
      summary.innerHTML = `<span>${esc(summaryText)}</span><span class="${warnClass}">${esc(fillTokens(tr('labChecksGradeBlockedRequired'), { passed: String(requiredPassed), total: String(requiredTotal) }))}</span>`;
    } else {
    ...
    body.innerHTML = rows.map((item) => {
      ...
      const required = !!(item && item.is_required);
      const requiredFailedRow = required && !item.is_passed;
      const requiredGlowStyle = requiredFailedRow
        ? 'box-shadow:0 0 0 1px rgba(244,63,94,0.35),0 0 10px rgba(244,63,94,0.35);'
        : '';
      return `<tr class="${theme() === 'light' ? 'border-b border-slate-100' : 'border-b border-white/10'} align-top">
        <td class="px-3 py-2">
          <div class="flex items-center gap-2">
            <span class="${checkClass}"${requiredGlowStyle ? ` style="${requiredGlowStyle}"` : ''}>${esc(titleText)}</span>
  ...
  async function loadConfig(showToast) {
    ...
    state.syncNotice = (data && data.sync_notice && typeof data.sync_notice === 'object' && !Array.isArray(data.sync_notice))
      ? data.sync_notice
      : null;
  ...
  async function syncChecksToCopies() {
    if (!state.canManage) {
      toast(tr('labChecksNoAccessConfig'), 'error');
      return;
    }
    const res = await ctx.jget(apiSyncCopies(), { method: 'POST' });
    const data = (res && res.data) ? res.data : {};
    const updated = Number(data.updated || 0);
    const reset = Number(data.reset_required || 0);
    const failed = Number(data.failed || 0);
    const msg = fillTokens(tr('labChecksSyncCopiesDone'), {
  ...
  async function resumeQueuedRunIfAny() {
    if (state.runBusy) return;
    let activeTask = null;
    try {
      const taskRes = await ctx.jget(apiLabTasks());
      const tasks = Array.isArray(taskRes && taskRes.data) ? taskRes.data : [];
      const candidate = selectTrackedLabCheckTask(tasks);
      if (candidate) {
        const status = String(candidate && candidate.status ? candidate.status : '').toLowerCase();
    ...
    state.runBusy = true;
    setRunBusyUi(true);
    state.runTaskId = String(activeTask && activeTask.id ? activeTask.id : '').trim();
    state.runTaskStatus = String(activeTask && activeTask.status ? activeTask.status : '').toLowerCase();
    state.runPollKnownIds = [];
    const createdAtMs = parseDateMs(activeTask && activeTask.created_at ? activeTask.created_at : '');
    state.runPollStartedAtMs = createdAtMs > 0 ? createdAtMs : Date.now();

    const taskResult = (activeTask && activeTask.result_data && typeof activeTask.result_data === 'object')
      ? activeTask.result_data
      : {};
    const taskRunId = String(taskResult && taskResult.run && taskResult.run.id ? taskResult.run.id : '').trim();
    state.runObservedId = taskRunId;
  ...
  async function saveConfig() {
    ...
    (Array.isArray(payload.items) ? payload.items : []).forEach((item, idx) => {
      const itemId = String(item && item.id ? item.id : '').trim();
      const collapsed = boolOf(item && item.ui_collapsed);
      if (itemId) {
        collapsedById.set(itemId, collapsed);
      }
      collapsedByIndex.set(idx, collapsed);
    });
    const savePayload = {
    ...
    if (Array.isArray(normalized.items) && normalized.items.length) {
      normalized.items = normalized.items.map((item, idx) => {
        const itemId = String(item && item.id ? item.id : '').trim();
        if (itemId && collapsedById.has(itemId)) {
          return { ...item, ui_collapsed: !!collapsedById.get(itemId) };
        }
        if (collapsedByIndex.has(idx)) {
          return { ...item, ui_collapsed: !!collapsedByIndex.get(idx) };
        }
        return item;
      });
    }
  ...
  async function runChecksNow() {
    if (state.runBusy || state.loading) return;
    ...
    renderRunLiveFeed();
    startRunProgress();
    setRunProgress(true, 2, tr('labChecksRunQueueing'));
    try {
      const res = await ctx.jget(`${apiBase()}/run`, { method: 'POST' });
      const data = (res && res.data) ? res.data : {};
      const runId = data && data.run && data.run.id ? String(data.run.id) : '';
      const taskId = data && data.task && data.task.id ? String(data.task.id) : '';
      const taskOwnerId = data && data.task && data.task.requested_by_user_id ? String(data.task.requested_by_user_id) : '';
      const meId = currentUserId();
      ...
      if (runId) {
        ...
        await finishRunBusyState(true, tr('labChecksRunDone'));
        return;
      }

      if (taskId) {
        state.runTaskId = taskId;
        setRunProgress(true, Math.max(4, Number(state.runProgressValue || 0)), tr('labChecksRunQueuedWait'));
      }
      const taskInProgress = String(data && data.reason ? data.reason : '').toLowerCase() === 'task_in_progress';
      if (taskInProgress && taskOwnerId && meId && taskOwnerId !== meId) {
        state.runBusy = false;
        setRunBusyUi(false);
        state.runPollKnownIds = [];
        state.runPollStartedAtMs = 0;
        ...
        renderRunLiveFeed();
        setRunProgress(false, 0, tr('labChecksRunInProgress'));
        toast(tr('labChecksRunAlreadyQueued'), 'success');
        return;
      }
      if (!taskId && !taskInProgress) {
        state.runBusy = false;
        setRunBusyUi(false);
        state.runPollKnownIds = [];
        state.runPollStartedAtMs = 0;
        ...
        renderRunLiveFeed();
        setRunProgress(false, 0, tr('labChecksRunInProgress'));
        throw new Error(tr('labChecksRunFailed'));
      }
      startRunPoller();
      if (taskInProgress) {
        setRunProgress(true, Math.max(4, Number(state.runProgressValue || 0)), tr('labChecksRunQueuedWait'));
        ...
        toast(tr('labChecksRunQueued'), 'success');
      }
    } catch (err) {
      stopRunPollTimer();
      state.runBusy = false;
  ...
  function buildConfigTemplatePayload(configRaw) {
    const normalized = normalizeConfigPayload(configRaw || {});
    const settings = {
      grading_enabled: !!(normalized.settings && normalized.settings.grading_enabled),
      pass_percent: numberOr(normalized.settings && normalized.settings.pass_percent, 60)
    };
    const grades = Array.isArray(normalized.grades) ? normalized.grades.map((grade) => ({
      grade_label: String((grade && grade.grade_label) || '').trim(),
      min_percent: numberOr(grade && grade.min_percent, 0)
    })).filter((grade) => grade.grade_label !== '') : [];
    const items = Array.isArray(normalized.items) ? normalized.items.map((item) => ({
      node_id: String((item && item.node_id) || ''),
      node_name: nodeNameById(item && item.node_id ? item.node_id : ''),
      title: String((item && item.title) || ''),
      transport: String((item && item.transport) || 'auto'),
      shell_type: String((item && item.shell_type) || 'auto'),
      command_text: String((item && item.command_text) || ''),
    ...
    return {
      template_type: 'eve_lab_checks_config',
      version: 1,
  ...
  function buildCurrentLabNodeLookup() {
    const nodes = Array.isArray(labData() && labData().nodes) ? labData().nodes : [];
    const byId = new Set();
    ...
    nodes.forEach((rawNode) => {
      ...
      if (!name) return;
      if (!byNameBuckets.has(name)) byNameBuckets.set(name, []);
      byNameBuckets.get(name).push(id);
    });

    const byNameUnique = new Map();
    byNameBuckets.forEach((ids, name) => {
      if (Array.isArray(ids) && ids.length === 1) {
        byNameUnique.set(name, String(ids[0] || ''));
      }
    });

    return { byId, byNameUnique };
  }

  function resolveImportedTemplateNodeId(rawItem, nodeLookup) {
    const src = rawItem && typeof rawItem === 'object' ? rawItem : {};
    const byId = nodeLookup && nodeLookup.byId instanceof Set ? nodeLookup.byId : new Set();
    const byNameUnique = nodeLookup && nodeLookup.byNameUnique instanceof Map ? nodeLookup.byNameUnique : new Map();

    const explicitId = String(src.node_id || '').trim();
    if (explicitId && byId.has(explicitId)) {
      return explicitId;
    }

    const byNameRaw = String(src.node_name || '').trim().toLowerCase();
    if (!byNameRaw) return '';
    const mappedId = String(byNameUnique.get(byNameRaw) || '').trim();
    return mappedId;
  }

  function normalizeConfigTemplatePayload(rawPayload) {
    ...
    const nodeLookup = buildCurrentLabNodeLookup();

    normalized.items = (Array.isArray(normalized.items) ? normalized.items : []).map((item, idx) => ({
      ...item,
      id: '',
      node_id: resolveImportedTemplateNodeId(rawItems[idx], nodeLookup),
      ui_collapsed: true
    }));
  ...
  async function exportConfigTemplate() {
    ...
    const template = buildConfigTemplatePayload(current);
    const blob = new Blob([JSON.stringify(template, null, 2)], { type: 'application/json;charset=utf-8' });
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    const fileName = `lab-checks-template-${stamp}.json`;
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = fileName;
    ...
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    toast(tr('labChecksConfigExportDone'), 'success');
  }

  async function importConfigTemplateFromFile(file) {
    ...
    try {
      ...
      throw new Error(tr('labChecksConfigImportInvalid'));
    }
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      throw new Error(tr('labChecksConfigImportInvalid'));
    }
    const payload = (parsed.payload && typeof parsed.payload === 'object' && !Array.isArray(parsed.payload)) ? parsed.payload : parsed;
    const hasShape = Array.isArray(payload.items) || Array.isArray(payload.grades) || (payload.settings && typeof payload.settings === 'object');
    if (!hasShape) {
      throw new Error(tr('labChecksConfigImportInvalid'));
    }
    const normalized = normalizeConfigTemplatePayload(parsed);
    state.config = normalized;
    renderManageSection();
    applyModalTheme();
    toast(tr('labChecksConfigImportDone'), 'success');
  }
  ...
  async function openModal() {
    ...
    if (workspace) {
      workspace.classList.add('pointer-events-none');
    }
    ...
    try {
      await loadConfig(false);
      if (Array.isArray(state.config.items) && state.config.items.length) {
      ...
      await loadRuns();
      await resumeQueuedRunIfAny();
    } finally {
      state.loading = false;
    }
  }

  function isGradesModalOpen() {
    const modal = byId('labChecksGradesModal');
    return !!(modal && !modal.classList.contains('hidden'));
  }

  function openGradesModal() {
    const modal = byId('labChecksGradesModal');
    if (!modal || !state.canManage || state.runBusy) return;
    modal.classList.remove('hidden');
    modal.classList.add('flex');
  }
  ...
  function isRegexModalOpen() {
    const modal = byId('labChecksRegexModal');
    return !!(modal && !modal.classList.contains('hidden'));
  }

  function regexRulePatternByType(type) {
    const mode = String(type || 'text').trim().toLowerCase();
    if (mode === 'number') return '\\d+';
    if (mode === 'ipv4') return '(?:\\d{1,3}\\.){3}\\d{1,3}';
    if (mode === 'interface') return '(?:[A-Za-z]+)?\\d+(?:\\/\\d+)+';
    if (mode === 'mac') return '(?:[0-9A-Fa-f]{2}(?::[0-9A-Fa-f]{2}){5}|[0-9A-Fa-f]{4}(?:\\.[0-9A-Fa-f]{4}){2})';
    if (mode === 'word') return '\\S+';
    if (mode === 'text') return '.+?';
    return '';
  }

  function detectRegexRuleTypeByLiteral(literalRaw) {
    const literal = String(literalRaw || '').trim();
    if (!literal) return 'text';
    if (/^(?:\d{1,3}\.){3}\d{1,3}$/.test(literal)) return 'ipv4';
    if (/^(?:[0-9A-Fa-f]{2}(?::[0-9A-Fa-f]{2}){5}|[0-9A-Fa-f]{4}(?:\.[0-9A-Fa-f]{4}){2})$/.test(literal)) return 'mac';
    if (/^(?:[A-Za-z]+)?\d+(?:\/\d+)+$/.test(literal.replace(/\s+/g, ''))) return 'interface';
    if (/^\d+$/.test(literal)) return 'number';
    if (/^\S+$/.test(literal)) return 'word';
    return 'text';
  }

  function normalizeRegexRuleType(type) {
    const mode = String(type || '').trim().toLowerCase();
    if (mode === 'number' || mode === 'ipv4' || mode === 'interface' || mode === 'mac' || mode === 'word' || mode === 'text' || mode === 'custom') {
      return mode;
    }
  ...
  function defaultRegexRule(literal, presetType, customPattern) {
    const rawLiteral = String(literal || '');
    const seedType = String(presetType || '').trim().toLowerCase();
    const detectedType = seedType === '' || seedType === 'auto' ? detectRegexRuleTypeByLiteral(rawLiteral) : normalizeRegexRuleType(seedType);
    const ruleType = detectedType === 'custom' ? 'custom' : detectedType;
    const fallbackPattern = ruleType === 'custom' ? '' : regexRulePatternByType(ruleType);
    return {
      literal: rawLiteral,
      type: ruleType,
      pattern: String(customPattern == null ? fallbackPattern : customPattern).trim() || fallbackPattern || '.+?'
    };
  }

  function regexRuleTypeOptionsHtml(selectedType) {
    const selected = normalizeRegexRuleType(selectedType);
    const rows = [
      { value: 'number', label: tr('labChecksRegexTypeNumber') },
      { value: 'ipv4', label: tr('labChecksRegexTypeIpv4') },
      { value: 'interface', label: tr('labChecksRegexTypeInterface') },
      { value: 'mac', label: tr('labChecksRegexTypeMac') },
      { value: 'word', label: tr('labChecksRegexTypeWord') },
      { value: 'text', label: tr('labChecksRegexTypeText') },
      { value: 'custom', label: tr('labChecksRegexTypeCustom') }
    ];
    return rows.map((row) => {
      const value = String(row.value || '');
      const label = String(row.label || value);
  ...
  function syncRegexPresetOptions() {
    const select = byId('labChecksRegexRuleTypePreset');
    if (!select) return;
    const current = String(select.value || 'auto').trim().toLowerCase() || 'auto';
    const rows = [
      { value: 'auto', label: tr('labChecksRegexTypeAuto') },
      { value: 'number', label: tr('labChecksRegexTypeNumber') },
      { value: 'ipv4', label: tr('labChecksRegexTypeIpv4') },
      { value: 'interface', label: tr('labChecksRegexTypeInterface') },
      { value: 'mac', label: tr('labChecksRegexTypeMac') },
    ...
    select.innerHTML = rows.map((row) => `<option value="${esc(row.value)}">${esc(row.label)}</option>`).join('');
    select.value = rows.some((row) => row.value === current) ? current : 'auto';
  }

  function setRegexHelperStatus(kind, text) {
    state.regexHelperStatus = {
  ...
  function renderRegexTestResult() {
    const box = byId('labChecksRegexTestResult');
    if (!box) return;
    const kind = String(state.regexHelperStatus && state.regexHelperStatus.kind ? state.regexHelperStatus.kind : 'idle');
    const text = String(state.regexHelperStatus && state.regexHelperStatus.text ? state.regexHelperStatus.text : '');
    if (theme() === 'light') {
      if (kind === 'ok') {
        box.className = 'mt-2 rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs text-emerald-700';
      } else if (kind === 'error') {
        box.className = 'mt-2 rounded-xl border border-rose-300 bg-rose-50 px-3 py-2 text-xs text-rose-700';
      } else {
        box.className = 'mt-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600';
      }
    } else if (kind === 'ok') {
      box.className = 'mt-2 rounded-xl border border-emerald-500/35 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-200';
    } else if (kind === 'error') {
      box.className = 'mt-2 rounded-xl border border-rose-500/35 bg-rose-500/10 px-3 py-2 text-xs text-rose-200';
    } else {
      box.className = 'mt-2 rounded-xl border border-slate-500/35 bg-slate-950/50 px-3 py-2 text-xs text-slate-300';
    }
    box.textContent = text || tr('labChecksRegexStatusIdle');
  }

  function collectRegexRulesFromDom() {
    const root = byId('labChecksRegexRulesList');
    if (!root) return;
    const next = [];
    root.querySelectorAll('.js-lab-check-regex-rule').forEach((row) => {
      const literalEl = row.querySelector('[data-regex-field="literal"]');
      const typeEl = row.querySelector('[data-regex-field="type"]');
      const patternEl = row.querySelector('[data-regex-field="pattern"]');
      const type = normalizeRegexRuleType(typeEl ? typeEl.value : 'text');
      const patternRaw = String(patternEl ? patternEl.value : '').trim();
      next.push({
        literal: String(literalEl ? literalEl.value : ''),
        type,
        pattern: type === 'custom'
          ? (patternRaw || '.+?')
          : (regexRulePatternByType(type) || '.+?')
      });
    });
    state.regexHelperRules = next;
  }

  function renderRegexRuleRows() {
    const root = byId('labChecksRegexRulesList');
    if (!root) return;
    const rules = Array.isArray(state.regexHelperRules) ? state.regexHelperRules : [];
    if (!rules.length) {
      root.innerHTML = `<div class="${theme() === 'light' ? 'rounded-xl border border-dashed border-slate-300 bg-white px-3 py-2 text-xs text-slate-600' : 'rounded-xl border border-dashed border-slate-500/35 bg-slate-950/40 px-3 py-2 text-xs text-slate-300'}">${esc(tr('labChecksRegexRulesEmpty'))}</div>`;
      return;
    }
    root.innerHTML = rules.map((rule, idx) => {
      const type = normalizeRegexRuleType(rule && rule.type);
      const resolvedPattern = type === 'custom'
        ? (String(rule && rule.pattern || '').trim() || '.+?')
        : (regexRulePatternByType(type) || '.+?');
      return `<div class="js-lab-check-regex-rule rounded-xl border p-2 ${theme() === 'light' ? 'border-slate-200 bg-white' : 'border-slate-500/35 bg-slate-950/40'}" data-rule-idx="${idx}">
        <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,170px)_minmax(0,1fr)_38px]">
          <input type="text" class="${inputClass({ compact: true, rounded: 'rounded-lg' })} h-8" data-regex-field="literal" value="${esc(String(rule && rule.literal || ''))}" placeholder="${esc(tr('labChecksRegexLiteralPlaceholder'))}">
          <select class="${inputClass({ compact: true, rounded: 'rounded-lg' })} h-8" data-regex-field="type">
            ${regexRuleTypeOptionsHtml(type)}
          </select>
          <input type="text" class="${inputClass({ compact: true, rounded: 'rounded-lg' })} h-8 ${type === 'custom' ? '' : 'opacity-70'}" data-regex-field="pattern" ${type === 'custom' ? '' : 'readonly'} value="${esc(resolvedPattern)}" placeholder="${esc(tr('labChecksRegexPatternPlaceholder'))}">
          <button type="button" class="js-lab-check-regex-del lab-btn inline-flex h-8 w-8 items-center justify-center rounded-xl border border-rose-300 bg-gradient-to-r from-rose-500 to-red-500 px-0 py-0 text-white hover:from-rose-600 hover:to-red-600" data-rule-del="${idx}">
            <i class="fa fa-trash"></i>
          </button>
        </div>
      </div>`;
  ...
  function regexEscapeText(value) {
    return String(value || '').replace(/[\\^$.*+?()[\]{}|]/g, '\\$&');
  }

  function regexReplacementNeedsGrouping(patternRaw) {
  ...
  function normalizeRegexReplacement(patternRaw) {
    const pattern = String(patternRaw || '').trim() || '.+?';
    return regexReplacementNeedsGrouping(pattern) ? `(?:${pattern})` : pattern;
  }

  function buildAutoRegexRulesFromSample(sampleTextRaw) {
    const sampleText = String(sampleTextRaw || '');
    const ranges = [];
    const rows = [];

    const intersects = (s, e) => ranges.some((r) => !(e <= r.start || s >= r.end));
    const pushMatch = (start, end, literal, type) => {
      if (start >= end) return;
      if (intersects(start, end)) return;
      ranges.push({ start, end });
      rows.push(defaultRegexRule(literal, type));
    };

    const addByRegex = (rx, type, transform) => {
      if (!(rx instanceof RegExp)) return;
      let match = rx.exec(sampleText);
      while (match) {
        const text = String(transform ? transform(match[0]) : match[0]);
        const start = Number(match.index || 0);
        const end = start + text.length;
        if (text.trim() !== '') {
          pushMatch(start, end, text, type);
        }
        if (rx.lastIndex === match.index) {
          rx.lastIndex += 1;
        }
        match = rx.exec(sampleText);
      }
    };

    addByRegex(/\b(?:\d{1,3}\.){3}\d{1,3}\b/g, 'ipv4');
    addByRegex(/\b(?:[0-9A-Fa-f]{2}(?::[0-9A-Fa-f]{2}){5}|[0-9A-Fa-f]{4}(?:\.[0-9A-Fa-f]{4}){2})\b/g, 'mac');
    addByRegex(/\b(?:GigabitEthernet|FastEthernet|Ethernet|TenGigabitEthernet|Serial|Loopback|Port-channel|Po|Gi|Fa|Eth|Te|Se|Lo)\s*\d+(?:\/\d+)+\b/gi, 'interface');
    addByRegex(/\b\d+\b/g, 'number');

    const unique = [];
    const seen = new Set();
    rows.forEach((rule) => {
      const key = `${String(rule.type || '')}\u0000${String(rule.literal || '')}`;
  ...
  function readRegexFlagsFromUi() {
    let flags = '';
    if (byId('labChecksRegexFlagI') && byId('labChecksRegexFlagI').checked) flags += 'i';
    if (byId('labChecksRegexFlagM') && byId('labChecksRegexFlagM').checked) flags += 'm';
    if (byId('labChecksRegexFlagS') && byId('labChecksRegexFlagS').checked) flags += 's';
    return flags;
  }

  function buildRegexFromHelperState() {
    const sampleInput = byId('labChecksRegexSampleInput');
    const sampleText = String(sampleInput ? sampleInput.value : '').replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    if (!sampleText.trim()) {
      throw new Error(tr('labChecksRegexErrNoSample'));
    }

    const tokens = [];
    let source = sampleText;
    const rules = Array.isArray(state.regexHelperRules) ? state.regexHelperRules : [];
    rules.forEach((rawRule, idx) => {
      const rule = rawRule && typeof rawRule === 'object' ? rawRule : {};
      const literal = String(rule.literal || '');
      if (!literal) return;
      const token = `__EVE_REGEX_TOKEN_${idx}__`;
      source = source.split(literal).join(token);
      tokens.push({
        token,
        replacement: String(rule.pattern || '').trim() || '.*?'
      });
    });

    let pattern = regexEscapeText(source);
    if (byId('labChecksRegexFlexibleSpaces') && byId('labChecksRegexFlexibleSpaces').checked) {
      pattern = pattern.replace(/[ \t]+/g, '\\s+');
    }
    tokens.forEach((item) => {
      const safeToken = regexEscapeText(item.token);
      pattern = pattern.split(safeToken).join(normalizeRegexReplacement(item.replacement));
    });

    const anchorStart = !!(byId('labChecksRegexAnchorStart') && byId('labChecksRegexAnchorStart').checked);
    const anchorEnd = !!(byId('labChecksRegexAnchorEnd') && byId('labChecksRegexAnchorEnd').checked);
    if (anchorStart) pattern = `^${pattern}`;
    if (anchorEnd) pattern = `${pattern}$`;

    const flags = readRegexFlagsFromUi();
    return { pattern, flags, sampleText };
  }

  function updateRegexPatternAndTest() {
    collectRegexRulesFromDom();
    const output = byId('labChecksRegexPatternOutput');
    try {
      const built = buildRegexFromHelperState();
      if (output) output.value = built.pattern;
      const testText = built.sampleText;
      const re = new RegExp(built.pattern, built.flags);
      const matched = re.test(testText);
      setRegexHelperStatus(matched ? 'ok' : 'error', matched ? tr('labChecksRegexStatusMatch') : tr('labChecksRegexStatusNoMatch'));
      return { pattern: built.pattern, flags: built.flags, matched };
    } catch (err) {
      if (output) output.value = '';
      setRegexHelperStatus('error', err && err.message ? err.message : tr('labChecksRegexStatusError'));
      return null;
    }
  }
  ...
  function openRegexModal() {
    ...
    const anchorStart = byId('labChecksRegexAnchorStart');
    const anchorEnd = byId('labChecksRegexAnchorEnd');
    const flexSpaces = byId('labChecksRegexFlexibleSpaces');
    const flagI = byId('labChecksRegexFlagI');
    const flagM = byId('labChecksRegexFlagM');
    const flagS = byId('labChecksRegexFlagS');
    if (anchorStart && anchorStart.checked !== true) anchorStart.checked = true;
    if (anchorEnd && anchorEnd.checked !== true) anchorEnd.checked = true;
    if (flexSpaces && flexSpaces.checked !== true) flexSpaces.checked = true;
    if (flagI && flagI.checked !== false) flagI.checked = false;
    if (flagM && flagM.checked !== false) flagM.checked = false;
    if (flagS && flagS.checked !== false) flagS.checked = false;
    syncRegexPresetOptions();

    if (!Array.isArray(state.regexHelperRules) || !state.regexHelperRules.length) {
      state.regexHelperRules = [defaultRegexRule('')];
    }
    renderRegexRuleRows();
    const output = byId('labChecksRegexPatternOutput');
    if (output) output.value = '';
    setRegexHelperStatus('idle', tr('labChecksRegexStatusIdle'));
    applyModalTheme();
  }
  ...
  function closeModal() {
    ...
    const workspace = byId('workspace');
    if (workspace) {
      workspace.classList.remove('pointer-events-none');
    }
    stopRunProgressTimer();
    stopRunPollTimer();
    state.runBusy = false;
  ...
  function bindEvents() {
    ...
    const modal = byId('labChecksModal');
    const modalCard = byId('labChecksModalCard');
    const modalBody = byId('labChecksModalBody');
    const itemsList = byId('labChecksItemsList');
    const resultsBox = byId('labChecksResultsBox');
    const historyBox = byId('labChecksHistoryBox');
    const saveBtn = byId('labChecksSaveBtn');
    ...
    const openGradesBtn = byId('labChecksOpenGradesModalBtn');
    const gradesCloseBtn = byId('labChecksGradesModalClose');
    const gradesBackdrop = byId('labChecksGradesModalBackdrop');
    const regexCloseBtn = byId('labChecksRegexModalClose');
    const regexBackdrop = byId('labChecksRegexModalBackdrop');
    const regexPresetSelect = byId('labChecksRegexRuleTypePreset');
    const regexAddRuleBtn = byId('labChecksRegexAddRuleBtn');
    const regexAddSelectionBtn = byId('labChecksRegexAddSelectionBtn');
    const regexAutoRulesBtn = byId('labChecksRegexAutoRulesBtn');
    const regexRulesList = byId('labChecksRegexRulesList');
    const regexBuildBtn = byId('labChecksRegexBuildBtn');
    const regexCopyBtn = byId('labChecksRegexCopyBtn');
    const regexSampleInput = byId('labChecksRegexSampleInput');
    ...
    const wheelLockHandler = (e) => {
      e.stopPropagation();
    };
    [modal, modalCard, modalBody, itemsList, resultsBox, historyBox].forEach((el) => {
      if (!el) return;
      el.style.overscrollBehavior = 'contain';
      el.addEventListener('wheel', wheelLockHandler, { passive: true });
    });
    ...
    if (configImportBtn) {
      configImportBtn.addEventListener('click', async () => {
        ...
        if (!file) return;
        try {
          await importConfigTemplateFromFile(file);
        } catch (err) {
          toast(err && err.message ? err.message : tr('labChecksConfigImportReadFailed'), 'error');
        }
      });
    }

    if (syncCopiesBtn) {
      syncCopiesBtn.addEventListener('click', async () => {
        try {
          await syncChecksToCopies();
        } catch (err) {
          toast(err && err.message ? err.message : tr('labChecksSyncCopiesFailed'), 'error');
        }
      });
    }
    ...
    if (gradesCloseBtn) {
      gradesCloseBtn.addEventListener('click', () => {
        closeGradesModal();
      });
    }
    if (gradesBackdrop) {
      gradesBackdrop.addEventListener('click', () => {
        closeGradesModal();
      });
    }

    if (regexHelperBtn) {
      regexHelperBtn.addEventListener('click', () => {
        openRegexModal();
      });
    }
    if (regexCloseBtn) {
      regexCloseBtn.addEventListener('click', () => {
        closeRegexModal();
      });
    }
    if (regexBackdrop) {
      regexBackdrop.addEventListener('click', () => {
        closeRegexModal();
      });
    }
    if (regexAddRuleBtn) {
      regexAddRuleBtn.addEventListener('click', () => {
        collectRegexRulesFromDom();
        const preset = String(regexPresetSelect ? regexPresetSelect.value : 'auto');
        state.regexHelperRules.push(defaultRegexRule('', preset));
        renderRegexRuleRows();
        applyModalTheme();
      });
    }
    if (regexAddSelectionBtn) {
      regexAddSelectionBtn.addEventListener('click', () => {
        const sample = byId('labChecksRegexSampleInput');
        if (!sample) return;
        const start = Number(sample.selectionStart || 0);
        const end = Number(sample.selectionEnd || 0);
        if (!Number.isInteger(start) || !Number.isInteger(end) || end <= start) {
          toast(tr('labChecksRegexSelectTextFirst'), 'error');
          return;
        }
        const selected = String(sample.value || '').slice(start, end);
        if (!selected) {
          toast(tr('labChecksRegexSelectTextFirst'), 'error');
          return;
        }
        collectRegexRulesFromDom();
        const preset = String(regexPresetSelect ? regexPresetSelect.value : 'auto');
        state.regexHelperRules.push(defaultRegexRule(selected, preset));
        renderRegexRuleRows();
        applyModalTheme();
        updateRegexPatternAndTest();
      });
    }
    if (regexAutoRulesBtn) {
      regexAutoRulesBtn.addEventListener('click', () => {
        const sample = byId('labChecksRegexSampleInput');
        const sampleText = String(sample ? sample.value : '');
        if (!sampleText.trim()) {
          toast(tr('labChecksRegexErrNoSample'), 'error');
          return;
        }
        const autoRules = buildAutoRegexRulesFromSample(sampleText);
        if (!autoRules.length) {
          toast(tr('labChecksRegexAutoRulesEmpty'), 'error');
          return;
        }
        state.regexHelperRules = autoRules;
        renderRegexRuleRows();
        applyModalTheme();
        updateRegexPatternAndTest();
        toast(fillTokens(tr('labChecksRegexAutoRulesDone'), { count: String(autoRules.length) }), 'success');
      });
    }
    if (regexRulesList) {
      regexRulesList.addEventListener('click', (e) => {
        const delBtn = e.target && e.target.closest ? e.target.closest('.js-lab-check-regex-del') : null;
        if (!delBtn) return;
        const idx = Number(delBtn.getAttribute('data-rule-del'));
        if (!Number.isInteger(idx) || idx < 0) return;
        collectRegexRulesFromDom();
        state.regexHelperRules.splice(idx, 1);
        if (!state.regexHelperRules.length) {
          state.regexHelperRules.push(defaultRegexRule(''));
        }
        renderRegexRuleRows();
        applyModalTheme();
        updateRegexPatternAndTest();
      });
      regexRulesList.addEventListener('input', () => {
        updateRegexPatternAndTest();
      });
      regexRulesList.addEventListener('change', (e) => {
        const target = e && e.target;
        if (!target) return;
        if (target.getAttribute('data-regex-field') !== 'type') {
          updateRegexPatternAndTest();
          return;
        }
        collectRegexRulesFromDom();
    ...
    if (regexBuildBtn) {
      regexBuildBtn.addEventListener('click', () => {
        updateRegexPatternAndTest();
      });
    }
    if (regexCopyBtn) {
      regexCopyBtn.addEventListener('click', async () => {
        const built = updateRegexPatternAndTest();
        const pattern = built && built.pattern ? String(built.pattern) : String((byId('labChecksRegexPatternOutput') && byId('labChecksRegexPatternOutput').value) || '').trim();
        if (!pattern) {
          toast(tr('labChecksRegexErrNoPattern'), 'error');
          return;
        }
        try {
          ...
          toast(tr('labChecksRegexCopyDone'), 'success');
        } catch (_) {
          toast(tr('labChecksRegexCopyFailed'), 'error');
        }
      });
    }
    [regexSampleInput,
      byId('labChecksRegexAnchorStart'),
      byId('labChecksRegexAnchorEnd'),
      byId('labChecksRegexFlexibleSpaces'),
      byId('labChecksRegexFlagI'),
      byId('labChecksRegexFlagM'),
      byId('labChecksRegexFlagS')
    ].forEach((el) => {
      if (!el) return;
  ...
  window.v2LabChecksI18nSync = function () {
    ...
    if (isRegexModalOpen()) {
      syncRegexPresetOptions();
      renderRegexRuleRows();
      const patternText = String((byId('labChecksRegexPatternOutput') && byId('labChecksRegexPatternOutput').value) || '').trim();
      if (patternText) {
        updateRegexPatternAndTest();
      } else {
        setRegexHelperStatus('idle', tr('labChecksRegexStatusIdle'));
