...
Mouse.prototype = {
    ...
    _releaseAllButtons: function () {
        if (!this._buttonMask) {
            return;
        }
        var pos = this._pos || { x: 0, y: 0 };
        for (var bit = 0; bit < 8; bit++) {
            var mask = 1 << bit;
            if ((this._buttonMask & mask) === 0) {
                continue;
            }
            this.onmousebutton(pos.x, pos.y, 0, mask);
        }
        this._buttonMask = 0;
    },
